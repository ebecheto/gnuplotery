...
set grid
set xlabel '$1=vds';set ylabel '$2=vgs';set zlabel 'IS(/MN0/D)@0' offset 4,4;replot
pause -1 'pressReturn'
...
set table "vdsat_07.dat"
splot  file  u 1:2:(abs($1-($2-Vth)*0.7)<0.001 ? (Zlast07=$3, $3):1/0) with lines
unset table
#pr Zlast
...
set label 3 at AAx*.9, AAy*.9, AAz "B" font 'Verdana,12' ;replot


#splot  file  with lines, "vdsat.dat", "diod.dat"
splot file u 1:2:($2>=Vth/2 && $2<=Vth ? $3:1/0) with lines,\
  file u 1:2:($2>=($1+Vth)/0.7-0.1 ? $3:1/0) with lines,\
  file u 1:2:($2<=($1+Vth)/0.7+0.1 && $2>=Vth ? $3:1/0) with lines,\
  file u 1:2:($2<=Vth/2.0 ? $3:1/0) with lines, "vdsat_07.dat" title 'vds=(vgs-vth)/0.7' ps 0.5, "diod.dat" title 'vds=vgs' ps 0.2,\

set arrow 11 from 0,Vth,zval to (3.3-Vth)*0.7,3.3,zval nohead;replot#<=vdsat projection
set arrow 15 from (3.3-Vth)*0.7,3.3,zval to (3.3-Vth)*0.7,3.3,Zlast nohead lt 5 ;replot
